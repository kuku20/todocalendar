...
if(isset($_POST['action']) or isset($_GET['view']))
{
    if(isset($_GET['view']))
    {
        ...
        $result = mysqli_query($link,"SELECT * FROM  `events` where (date(start) >= '$start' AND date(start) <= '$end') and userEvent='".$userEvent."'");
    ...
    {   
        mysqli_query($link,"INSERT INTO `events` (
                    ...
                    '".mysqli_real_escape_string($link,date('Y-m-d',strtotime($_POST["start"])))."',
                    '".mysqli_real_escape_string($link,date('Y-m-d',strtotime($_POST["end"])))."'
                    )");
        header('Content-Type: application/json');
    ...
    elseif($_POST['action'] == "update")
    {
        mysqli_query($link,"UPDATE `events` set 
            ...
            `datestart` = '".mysqli_real_escape_string($link,date('Y-m-d',strtotime($_POST["start"])))."',
            `dateend`= '".mysqli_real_escape_string($link,date('Y-m-d',strtotime($_POST["end"])))."' 
